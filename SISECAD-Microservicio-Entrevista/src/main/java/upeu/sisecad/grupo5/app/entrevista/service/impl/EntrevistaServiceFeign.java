...
public class EntrevistaServiceFeign implements EntrevistaService {
	...
	public List<Entrevista> findAll() {
		...
		return clienteFeign.all().stream().map(c -> new Entrevista(
				entrevistaDao.findByIdCronograma(c.getId_cronograma()).orElse(null).getId_entrevista(),
				c,
				entrevistaDao.findByIdCronograma(c.getId_cronograma()).orElse(null).getEn_entrevista()
